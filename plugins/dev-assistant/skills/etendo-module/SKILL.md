...
**Important:** Create the directory structure now, but do NOT generate the XML files yet. The UUID for AD_MODULE_ID is generated by the DB in Step 4 (via webhook or SQL). After Step 4 completes and you have the MODULE_ID, write it into the XML files. This ensures the XML and DB are in sync.
