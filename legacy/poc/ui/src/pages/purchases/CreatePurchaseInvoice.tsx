/**
 * CreatePurchaseInvoice.tsx
 *
 * Form to create a purchase invoice in Etendo Lite.
 * Vendor search + date + product lines (search + qty + unit price).
 * Tax and UOM are resolved server-side automatically.
 *
 * API: POST /lite/purchases/invoices
 *   Body: { vendorId, date?, description?, lines: [{ productId, quantity, unitPrice }] }
 */
...
function VendorSearch({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      setLoading(true)
      try {
        const r = await apiFetch(`${apiBase}/masters/vendors/search?q=${encodeURIComponent(query)}`, { headers })
  ...
  function select(v: Vendor) { onChange(v); setQuery(v.name); setOpen(false) }
      ...
      {open && results.length > 0 && (
        <div style={{
          ...
          {results.map(v => (
            ...
            >
              <span style={{ fontWeight: 500, color: C.textDark, fontSize: 13 }}>{v.name}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 11, color: C.textMuted }}>{v.code}</span>
            </div>
          ))}
        </div>
      )}
...
export default function CreatePurchaseInvoice({ apiBase = '/lite', tenantId, token }: AuthProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      const r = await apiFetch(`${apiBase}/purchases/invoices`, {
        method: 'POST',
        headers: {
      ...
      <div style={{ padding: '32px 40px', maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 80, gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
          <button
            ...
            onClick={() => { setResult(null); setVendor(null); setLines([{ id: uid(), product: null, productSearch: '', quantity: 1, unitPrice: 0 }]); setDescription('') }}
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        ...
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: C.textDark, margin: 0 }}>New Purchase Invoice</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ ...s.card, padding: '24px 28px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              <label style={s.label}>Vendor <span style={{ color: C.danger }}>*</span></label>
              <VendorSearch
