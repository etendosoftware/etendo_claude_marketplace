---
name: "etendo:java"
description: "/etendo:java — Crear codigo Java en un modulo Etendo"
argument-hint: "<descripcion, ej: 'eventhandler para SMFT_Inscripcion'>"
---

# /etendo:java — Crear codigo Java en un modulo Etendo
...
Resolver modulo activo y detectar el tipo de componente a crear:

| Tipo | Cuando |
|---|---|
| **EventHandler** | "cuando se guarda/crea/modifica X, hacer Y" |
| **Background Process** | "proceso que corre todos los dias / periodicamente" |
| **Action Process** | "proceso lanzable desde menu o boton con parametros" |
| **Webhook** | "exponer una operacion como endpoint HTTP llamable por Copilot" |
| **Computed Column** | "campo calculado en una ventana existente" |
| **Callout** | "logica al cambiar un campo en la UI" |

## Step 2: Entender que se necesita

Preguntar solo lo necesario:
- Que entidad/tabla es el sujeto?
- Que debe hacer exactamente?
- Que validaciones hay?
- Que parametros recibe (para Action Process)?

## Step 3: Descubrir el package de las entidades generadas

**CRITICO**: Antes de escribir codigo Java, verificar el package real de las entidades generadas:
...
El package generado por Etendo sigue el patron: `{javapackage}.{javapackage}.ad`
...
**Si las entidades no existen todavia** (tabla recien creada), ejecutar primero:
```bash
./gradlew resources.up && sleep 15
JAVA_HOME=... ./gradlew generate.entities
```

## Step 4: Escribir el codigo Java
...
public class {Entity}EventHandler extends EntityPersistenceEventObserver {
  ...
  private void handle({Entity} record) {
    // logica aqui -- usar record.getXxx() / record.setXxx()
...
**Registrar en AD** via webhook (Tomcat debe estar UP):
```bash
...
### Action Process (lanzable desde menu)
...
Un webhook es una clase Java que extiende `BaseWebhookService` y expone una operacion HTTP
llamable por Copilot u otros clientes. Requiere registro en BD despues de crear el archivo.

**Patron del archivo:** `modules/{module}/src/{javapackage}/webhooks/{Name}.java`
...
/**
 * Webhook to {descripcion de que hace}.
 *
 * <p>Required parameters:
 * <ul>
 *   <li>{@code Param1} -- descripcion</li>
 * </ul>
 * <p>Optional parameters:
 * <ul>
 *   <li>{@code Param2} -- descripcion (default: valor)</li>
 * </ul>
...
**Despues de crear el archivo**, registrar en BD via webhook (Tomcat debe estar UP):
...
> Si necesitas params opcionales, ajustarlos manualmente en la ventana de Webhooks del AD.
>
...
> siguiendo el patron de los webhooks existentes del modulo.

## Step 5: Registrar en AD (si no se uso webhook)
...
3. Verificar que las entidades existen (generate.entities corrio exitosamente)
...
+ {Tipo} {Nombre} creado y compilado
  ...
  Proximos pasos:
    /etendo:smartbuild -> si aun no corrio
