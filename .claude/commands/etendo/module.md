...
## Step 4: Register in the DB

```sql
DO $$
DECLARE
  v_module_id TEXT := REPLACE(gen_random_uuid()::text, '-', '');
  v_prefix_id TEXT := REPLACE(gen_random_uuid()::text, '-', '');
BEGIN
  ...
  -- Print the generated module ID so we can use it
  RAISE NOTICE 'MODULE_ID: %', v_module_id;

END $$;
```

Execute this, capture the generated `v_module_id` from the NOTICE output, and write it into the XML files.
